...
path = "/home/clement/DATA/Multimedia/Documents/STAGE/Bradford/Travail/dataset"
...
def load_data(dataset, l, image_height=256, image_width=256):
    name = path + dataset
    name = name.strip()
    directory = Path(name)
    listFile = list(directory.iterdir())
    x = np.zeros((l, image_height, image_width, 3))
    y = np.empty(l)
    k = 0
...
def train(epochs, batch_size=128, save_interval=50):
    ...
    # Load the dataset
    (X_train, _) = load_data('/images', 13000, 512, 512)
